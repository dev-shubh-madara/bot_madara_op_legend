...
State machine per user:
  idle  →  awaiting_phone  →  awaiting_otp  →  [awaiting_2fa]  →  done
"""
import asyncio
import logging
import telebot
...
def _cleanup(uid: int):
    """Disconnect and remove temp client from state."""
...
def register(bot: telebot.TeleBot):
    ...
    def gen_cmd(msg: telebot.types.Message):
        ...
        if existing and client:
            ...
            bot.send_message(
                msg.chat.id,
                f"⚠️ ʏᴏᴜ ᴀʟʀᴇᴀᴅʏ ʜᴀᴠᴇ ᴀɴ ᴀᴄᴛɪᴠᴇ ᴜꜱᴇʀʙᴏᴛ.\n"
    ...
    def gen_replace_cb(call: telebot.types.CallbackQuery):
        ...
        if call.data == "gen_keep":
            bot.edit_message_text("✅ ᴋᴇᴩᴛ ʏᴏᴜʀ ᴇxɪꜱᴛɪɴɢ ᴜꜱᴇʀʙᴏᴛ.", call.message.chat.id, call.message.message_id)
            return
    ...
    def cancel_gen(msg: telebot.types.Message):
        ...
        if uid in _pending_add:
            ...
            cleared = True
        bot.send_message(msg.chat.id, "❌ ᴄᴀɴᴄᴇʟʟᴇᴅ." if cleared else "❌ ɴᴏᴛʜɪɴɢ ᴛᴏ ᴄᴀɴᴄᴇʟ.")
...
def _handle_phone(bot: telebot.TeleBot, msg: telebot.types.Message, uid: int, entry: dict):
    ...
    wait = bot.send_message(chat_id, "⏳ ꜱᴇɴᴅɪɴɢ ᴏᴛᴩ…")
    ...
    try:
        ...
        bot.edit_message_text("❌ ɪɴᴠᴀʟɪᴅ ᴩʜᴏɴᴇ ɴᴜᴍʙᴇʀ. ᴛʀʏ ᴀɢᴀɪɴ.", chat_id, wait.message_id)
        return
    except Exception as e:
        _cleanup(uid)
        bot.edit_message_text(f"❌ ᴇʀʀᴏʀ: {e}\n\nᴜꜱᴇ /gen ᴛᴏ ʀᴇᴛʀʏ.", chat_id, wait.message_id)
        return
...
def _handle_otp(bot: telebot.TeleBot, msg: telebot.types.Message, uid: int, entry: dict):
    ...
    wait = bot.send_message(chat_id, "⏳ ᴠᴇʀɪꜰʏɪɴɢ ᴏᴛᴩ…")
    ...
    try:
        ...
        bot.edit_message_text("❌ ᴡʀᴏɴɢ ᴏᴛᴩ. ᴩʟᴇᴀꜱᴇ ꜱᴇɴᴅ ᴛʜᴇ ᴄᴏʀʀᴇᴄᴛ ᴄᴏᴅᴇ.", chat_id, wait.message_id)
    except PhoneCodeExpired:
        _cleanup(uid)
        bot.edit_message_text("❌ ᴏᴛᴩ ᴇxᴩɪʀᴇᴅ. ᴜꜱᴇ /gen ᴛᴏ ꜱᴛᴀʀᴛ ᴀɢᴀɪɴ.", chat_id, wait.message_id)
    except Exception as e:
        _cleanup(uid)
        bot.edit_message_text(f"❌ ᴇʀʀᴏʀ: {e}\n\nᴜꜱᴇ /gen ᴛᴏ ʀᴇᴛʀʏ.", chat_id, wait.message_id)
...
def _handle_2fa(bot: telebot.TeleBot, msg: telebot.types.Message, uid: int, entry: dict):
    ...
    wait = bot.send_message(chat_id, "⏳ ᴠᴇʀɪꜰʏɪɴɢ 2ꜰᴀ…")
    ...
    try:
        ...
        bot.edit_message_text("❌ ᴡʀᴏɴɢ ᴩᴀꜱꜱᴡᴏʀᴅ. ᴩʟᴇᴀꜱᴇ ᴛʀʏ ᴀɢᴀɪɴ.", chat_id, wait.message_id)
    except Exception as e:
        _cleanup(uid)
        bot.edit_message_text(f"❌ ᴇʀʀᴏʀ: {e}\n\nᴜꜱᴇ /gen ᴛᴏ ʀᴇᴛʀʏ.", chat_id, wait.message_id)
...
def _finish_login(bot: telebot.TeleBot, chat_id: int, uid: int, entry: dict, edit_msg_id: int):
    """Export session string, store in DB, start live client."""
    client: Client = entry["client"]
    ...
    try:
        ...
        _cleanup(uid)
        bot.edit_message_text(f"❌ ꜰᴀɪʟᴇᴅ ᴛᴏ ᴇxᴩᴏʀᴛ ꜱᴇꜱꜱɪᴏɴ: {e}", chat_id, edit_msg_id)
        return

    # Remove temp state (client already disconnected)
    _state.pop(uid, None)

    # Store in DB and start live client
    username = getattr(me, "username", "") or ""
    first_name = getattr(me, "first_name", "") or ""
    add_userbot(uid, session_string, username, first_name)

    # Start the live persistent client
    async def _start():
